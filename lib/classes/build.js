...
Build = (function() {
  ...
  Build.prototype.build = function(defaults) {
    ...
    if (this.pre_install_cmd === null || this.install_cmd === null || this.post_install_cmd === null) {
      warnings = true;
    }
    ...
    if (this.pre_build_cmd === null || this.build_cmd === null || this.post_build_cmd === null) {
      warnings = true;
    }
    ...
    if (this.post_build_cmd && this.post_build_args) {
      this.logger.info("Running post_build command...");
  ...
  Build.prototype._initEnv = function() {
    var env, env_setup, i, j, len, len1, prop, ref, ref1, results, split_prop;
    this.env = {
  ...
  Build.prototype.exec = function(cmd, args) {
    var err, err_string, i, j, len, len1, ref, ref1, stderr, stdout;
    ...
    if (this.options.hasOwnProperty('showOutput') && this.options.showOutput === true) {
    ...
    this.logger.info("Process exited with " + (this.output.status.toString().yellow.bold) + ".");
    if (this.output.status > 0) {
      ...
      this.logger.err("");
      return this.fail(new Error("Command \'" + cmd + " " + (args.toString().replace(/,/g, ' ')) + "\' exited with " + this.output.status + "!"));
    }
  };
