 ...
 * Module dependencies
 */
var CSON, Logger, _finishExtract, fs, get, os, path, retis_plugin_dir, targz, unpack, unzip;
...
_finishExtract = function(config, logger, callback) {
  ...
  return fs.rename(retis_plugin_dir + "/.tmp/extract/" + config.extract_dir, retis_plugin_dir + "/.tmp/extract/" + config.name, function(err) {
    if (err) {
      if (typeof callback !== 'undefined') {
        callback(err);
      } else {
